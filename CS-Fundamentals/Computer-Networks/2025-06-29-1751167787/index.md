转载自: [网络编程篇：VPN 原理与实战开发](https://xzcoder.com/posts/network/05-simple-vpn.html)

- [网络编程篇：VPN 原理与实战开发](#网络编程篇vpn-原理与实战开发)
  - [一、背景介绍](#一背景介绍)
  - [二、VPN 客户端工作原理](#二vpn-客户端工作原理)
  - [三、VPN 服务端端工作原理](#三vpn-服务端端工作原理)
  - [四、实战开发](#四实战开发)
    - [1、VPN 客户端开发](#1vpn-客户端开发)
      - [（1）定义程序输入参数](#1定义程序输入参数)
      - [（2）定义结构体存储客户端信息](#2定义结构体存储客户端信息)
      - [（3）与服务端创建 TCP 连接](#3与服务端创建-tcp-连接)
      - [（4）向服务端注册虚拟 IP 并得到旁路地址](#4向服务端注册虚拟-ip-并得到旁路地址)
      - [（5）创建 Tun 设备并转发数据包](#5创建-tun-设备并转发数据包)
      - [（6）处理服务端返回的数据包](#6处理服务端返回的数据包)
    - [2、VPN 服务端开发](#2vpn-服务端开发)
      - [（1）定义程序输入参数](#1定义程序输入参数-1)
      - [（2）定义结构体存储服务端信息](#2定义结构体存储服务端信息)
      - [（3）监听 TCP 端口接收客户端连接](#3监听-tcp-端口接收客户端连接)
      - [（4）处理客户端注册指令数据包](#4处理客户端注册指令数据包)
      - [（5）创建 Tun 设备并转发数据包](#5创建-tun-设备并转发数据包-1)
      - [（6）处理客户端转发指令数据包](#6处理客户端转发指令数据包)
  - [五、程序测试](#五程序测试)
    - [1、测试环境准备](#1测试环境准备)
    - [2、访问验证](#2访问验证)
  - [六、总结](#六总结)
...
![网络拓扑图](assets/1751167787-766192db6b3c46761a06111c0a7369c7.png)
...
![VPN客户端发送数据](assets/1751167787-12120f83a77ba1ab237483c974df5d18.png)
...
![VPN客户端接收数据](assets/1751167787-914a1178be953708bdaa67569d00742b.png)
...
![VPN程序截图](assets/1751167787-1f7714dd882f9ecf1f99e9687fc09fc0.png)

### 2、访问验证

**`ping`命令访问`nginx`容器**

![ping局域网机器](assets/1751167787-23523625f4a40c3fe0dd1715453d444b.png)

**`curl`访问`nginx`容器的`80`端口**

![curl局域网端口](assets/1751167787-fd38db40361af7485cfb6caf6a31f1cb.png)
